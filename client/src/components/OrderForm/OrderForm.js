...
class OrderForm extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      assetPricePerShare: this.props.instrument.highestAsk,
      numberOfShares: DEFAULT_NB_OF_SHARES,
      usdxAmount: this.props.instrument.highestAsk * DEFAULT_NB_OF_SHARES,
    };
  }
  ...
  handleSubmit = () => {
    ...
    const orderEntry = {
      ...
      price: this.state.assetPricePerShare,
      totalPrice: this.state.usdxAmount,
      side: this.state.marketSideIsBuy ? "BUY" : "SELL",
    };
    onSubmit(orderEntry);
  };
  ...
  render() {
    ...
    return (
      <DashboardCard title="Order Form">
        ...
        >
          <Button
            onClick={() => this.toggleMarketSide(instrument)}
            color={this.state.marketSideIsBuy ? "success" : "inactive"}
          >
            Buy
          </Button>
          <Button
            onClick={() => this.toggleMarketSide(instrument)}
            color={this.state.marketSideIsBuy ? "inactive" : "error"}
          >
            Sell
          </Button>
          <Info
          ...
          />
          <input type="hidden" name="asset-id" value={instrument.id} />
          <input
            type="hidden"
            name="side"
            value={this.state.marketSideIsBuy ? "bid" : "ask"}
          />
          <Button
            type="submit"
            color={this.state.marketSideIsBuy ? "success" : "error"}
          >
            {this.state.marketSideIsBuy ? "Buy Assets" : "Sell Assets"}
          </Button>
        </Form>
      </DashboardCard>
    );
  ...
  toggleMarketSide = (instrument) => {
    this.setState((prevState, props) => {
      ...
      const usdxAmount = prevState.numberOfShares * assetPricePerShare;
